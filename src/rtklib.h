...
typedef struct {        /* observation data record */
    ...
    uint8_t sat,rcv;    /* satellite/receiver number sat：卫星编号 */
    ...
    double L[NFREQ+NEXOBS]; /* observation data carrier-phase (cycle)  NEXOBS 扩展*/
...
} obsd_t;  // 单个卫星的观测值的存储结构

typedef struct {        /* observation data */
    int n,nmax;         /* number of observation data/allocated n:观测值个数 nmax:最大的卫星数 */
    int flag;           /* epoch flag (0:ok,1:power failure,>1:event flag) */
    int rcvcount;       /* count of rcv event */
    int tmcount;        /* time mark count */
    obsd_t *data;       /* observation data records  用来存储多个观测数据 */
} obs_t;        // 存了一个历元的观测值文件
...
typedef struct {        /* antenna parameter type  天线参数类型 */
...
typedef struct {        /* almanac type 历书*/
...
} tled_t;   // https://wenku.csdn.net/answer/3mi5y96cgk  表示卫星的轨道状态
...
} ssr_t; // https://blog.csdn.net/weixin_42918498/article/details/121665635
...
typedef struct {        /* navigation data type */
    ...
    int glo_fcn[32];    /* GLONASS FCN + 8 */  // FCN是频道编号（‌Frequency Channel Number）‌的缩写，‌用于标识GLONASS卫星信号的特定频道。
...
} nav_t;  // 一个nav里存了整个 nav，sp3，clk文件的内容
...
typedef struct {        /* solution type */
    ...
    float thres;        /* AR ratio threshold for validation */ // AR ratio 阈值
...
typedef struct {        /* option type */
    ...
    int format;         /* option format (0:int,1:double,2:string,3:enum) =single 等号后面的内容 */
    void *var;          /* pointer to option variable */
    const char *comment; /* option comment/enum labels/unit 注释 */
} opt_t;
...
typedef struct {        /* RTK control/result type */
    ...
    double *x, *P;      /* float states and their covariance EFK的x阵和p阵
                           x阵，xyz + 若干个 dtr* + 对流层 + G01...G32模糊度 及所以系统卫星的模糊度
                           *，例如只结算GPS，但预定义里开启了4个系统，则会出现4和相同的dtr（后三个都=首个GPS系统的dtr），此处dtr都是由spp而来
                         */
