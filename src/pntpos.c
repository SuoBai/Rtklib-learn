    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */
        ...
        *var=SQR(ERR_CBIAS);
        ...
        if (sys==SYS_GPS||sys==SYS_QZS) { /* L1 */
            ...
            // 北斗的n文件的卫星钟差都是基于B3频点求得
            if      (obs->code[0]==CODE_L2I) b1=gettgd(sat,nav,0); /* TGD_B1I */  // 当前频段与B3频段的差异
            else if (obs->code[0]==CODE_L1P) b1=gettgd(sat,nav,2); /* TGD_B1Cp */
            else b1=gettgd(sat,nav,2)+gettgd(sat,nav,4); /* TGD_B1Cp+ISC_B1Cd */
            return P1-b1; // tgd 改正可以加到钟差上，也可以加到伪距上 单位：米
        }
                    ...
                    const double *azel, int ionoopt, double *ion, double *var)
{
    ...
    /* GPS broadcast ionosphere model Klobuchar模型 spp都会执行这步 */
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    for (i=0;i<3;i++) rr[i]=x[i]; // （经迭代后的）测站xyz
    dtr=x[3];
    
    ecef2pos(rr,pos); // ECEF（地心地固（xyz）坐标）转大地坐标 BLH
    trace(3,"rescode: rr=%.3f %.3f %.3f\n",rr[0], rr[1], rr[2]);
    
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        vsat[i]=0; azel[i*2]=azel[1+i*2]=resp[i]=0.0; // 定位有效， azi和ele,伪距残差置0
        time=obs[i].time;
        sat=obs[i].sat;
        if (!(sys=satsys(sat,NULL))) continue; // 验证卫星编号是否合理及其所属的导航系统
        
        /* reject duplicated observation data */
        if (i<n-1&&i<MAXOBS-1&&sat==obs[i+1].sat) { // 检测当前的观测值是否和下一个相邻数据重复
            trace(2,"duplicated obs data %s sat=%d\n",time_str(time,3),sat);
            i++;
            continue;
        }
        /* excluded satellite? */
        if (satexclude(sat,vare[i],svh[i],opt)) continue;  // 剔除坏卫星（匹配不到星历的卫星）
        
        /* geometric distance and elevation mask*/
        if ((r=geodist(rs+i*6,rr,e))<=0.0) continue; // 计算卫地距r + 地->卫 方向单位向量e
        if (satazel(pos,e,azel+i*2)<opt->elmin) continue; // 计算azi和ele （方位角和高度角）
        
        if (iter>0) { // 当前历元的首次迭代
            /* test SNR mask */
            if (!snrmask(obs+i,azel+i*2,opt)) continue; // 验证信号质量
        
            /* ionospheric correction */
            if (!ionocorr(time,nav,sat,pos,azel+i*2,opt->ionoopt,&dion,&vion)) {
                continue; // 电离层改正 里面有各种模型
            }
            if ((freq=sat2freq(sat,obs[i].code[0],nav))==0.0) continue; // 获取当前载波频率，以计算当前载波的dion和方差
            /* Convert from FREQL1 to freq */
            dion*=SQR(FREQL1/freq);  // 原dion是基于GPS L1载波求得的！！若当前载波非GPS L1 则还需要 * (L1^2 / 当前载波^2)
        ...
        if ((P=prange(obs+i,nav,opt,&vmeas))==0.0) continue; // 修正伪距（DCB/Tgd改正） vmeas:dcb误差
        ...
        /* design matrix */
        for (j=0;j<NX;j++) { // H 前3行为单位向量e的反方，因为e是卫星-接收机，伪距方程是接收机-卫星；第4行为1；其他行为0
            H[j+nv*NX]=j<3?-e[j]:(j==3?1.0:0.0); // 此处的H是正常H的专职；c语言H[1] 指向2行1列！！  为了加快计算
        }
        /* time system offset and receiver bias correction */
        if      (sys==SYS_GLO) {v[nv]-=x[4]; H[4+nv*NX]=1.0; mask[1]=1;} // x[4]：glo-gps time offset
        else if (sys==SYS_GAL) {v[nv]-=x[5]; H[5+nv*NX]=1.0; mask[2]=1;} // x[5]：bds-gps time offset
        else if (sys==SYS_CMP) {v[nv]-=x[6]; H[6+nv*NX]=1.0; mask[3]=1;} // x[6]：gal-gps time offset
        else if (sys==SYS_IRN) {v[nv]-=x[7]; H[7+nv*NX]=1.0; mask[4]=1;}
#ifdef QZSDT
        else if (sys==SYS_QZS) {v[nv]-=x[8]; H[8+nv*NX]=1.0; mask[5]=1;}
#endif
        // mask ：预编译器启用系统的mask为1，未启用的为0
        else mask[0]=1; 

        vsat[i]=1; resp[i]=v[nv]; (*ns)++;
        
        /* variance of pseudorange error */ // 计算伪距方差 = 本系统固有方差+卫星位置&钟差方差+伪距偏移方差+电离层+对流层+
        var[nv]=vare[i]+vmeas+vion+vtrp;    // var[i] 过大，表明第i个方程不可信 （定权时降权处理？）
        if (ssat)
    ...
    /* constraint to avoid rank-deficient  避免秩亏*/
    /* 为防止秩亏添加人工方程 0 = 0*e+1*dtr+0*其它，解得未启用系统的dtr=0， 但若不加此方程会导致未知数个数>方程个数（eg,只启用北斗，但是）
    
    */
    // for会使得nv+4，即添加4行全为0方程，使得实际可见卫星数=nv-4
    for (i=0;i<NX-3;i++) {
        if (mask[i]) continue; // 已启用的系统，都建立方程，跳；此处给为启用的系统添加人工方程
        v[nv]=0.0; // 人工方程的残差 注意nv不是从0开始，but从ns开始
        for (j=0;j<NX;j++) H[j+nv*NX]=j==i+3?1.0:0.0; // 改变H 第nv+1行（添加新方程）， dtr系统 = 1，其余全0
        var[nv++]=0.01; // 人工方程的方差
    }
    return nv;  // 放回方程的个数
    // v和 resp的区别; 前者nv+1，后者n*1（nv<=n, n可能包含坏卫星等 continue 情况）
}
/* validate solution  对伪距残余进行卡方检验 + GDOP检验-----------------------------------------------------*/
                  ...
                  char *msg)
{
    ...
    /* Chi-square validation of residuals 卡方检验 */
    vv=dot(v,v,nv);
    if (nv>nx&&vv>chisqr[nv-nx-1]) { // 定位误差过大 跳
    ...
    /* large GDOP check */
    for (i=ns=0;i<n;i++) { 
        if (!vsat[i]) continue; // 复制参与定位的卫星 azels
        azels[  ns*2]=azel[  i*2];
        azels[1+ns*2]=azel[1+i*2];
        ns++; // 参与定位的卫星个数
    }
    dops(ns,azels,opt->elmin,dop); // 计算各种精度因子（DOP）
    if (dop[0] <= 0.0 || dop[0] > MAX_GDOP) { // 只确保首个精度因子GPOP∈(0,max)
                  ...
                  int *vsat, double *resp, char *msg)
{
    ...
    v=mat(n+NX-3,1); H=mat(NX,n+NX-3); var=mat(n+NX-3,1); // H:8行（未知数个数）
    
    for (i=0;i<3;i++) x[i]=sol->rr[i]; // 首历元，sol为空，x=0; 非首历元，取上一历元的结果作为初值

    for (i=0;i<MAXITR;i++) {

        /* pseudorange residuals (m) */
        // 计算伪距残差v（&方差var） + 构建H阵； v = Hx + e; x阵：xyz+dtr v阵：加了各项改正项的伪距-卫地距
        nv=rescode(i,obs,n,rs,dts,vare,svh,nav,x,opt,ssat,v,H,var,azel,vsat,resp,&ns); // H是转置了的
        
        if (nv<NX) { // 方程个数 < 未知数个数 跳
            sprintf(msg,"lack of valid sats ns=%d",nv);
            break;
        }
        /* weight by variance (lsq uses sqrt of weight */
        // 方差定权 以伪距残余的标准差的倒数作为权重，对H 和v分别 左乘权重对角阵；得到加权之后的 H和v
        for (j=0;j<nv;j++) {
            sig=sqrt(var[j]); // 标准差 
            v[j]/=sig; // 权 = 1 / std 相当于左乘
            for (k=0;k<NX;k++) H[k+j*NX]/=sig;
        }
        /* least square estimation  最小二乘*/
        if ((info=lsq(H,v,NX,nv,dx,Q))) { // H 是转置了的
        ...
        if (norm(dx,NX)<1E-4) { // 当改正数足够小时，存储结果，结束迭代
            sol->type=0; //  坐标采用ecef-xyz
            sol->time=timeadd(obs[0].time,-x[3]/CLIGHT); // 去除了dtr后的 信号发射瞬间的卫星钟面时刻
            sol->dtr[0]=x[3]/CLIGHT; /* receiver clock bias (s) */
            sol->dtr[1]=x[4]/CLIGHT; /* GLO-GPS time offset (s) */ // 单系统定位以下四个都是0 无用
...
#endif
            for (j=0;j<6;j++) sol->rr[j]=j<3?x[j]:0.0; // xyz赋值
            for (j=0;j<3;j++) sol->qr[j]=(float)Q[j+j*NX]; // Q阵对角线元素即为方差
            ...
            if ((stat=valsol(azel,vsat,n,opt,v,nv,NX,msg))) { // 返回值stat :0 不通过 1-通过
                sol->stat=opt->sateph==EPHOPT_SBAS?SOLQ_SBAS:SOLQ_SINGLE; // ppp的spp此步会=SOLQ_SINGLE
            }
            free(v); free(H); free(var);
            return stat; // 精度理想定位，到此就结束返回了
        }
    }
    if (i>=MAXITR) sprintf(msg,"iteration divergent i=%d",i);
    
    free(v); free(H); free(var);
    return 0; //来到这里说明精度不满足要求
}
                  ...
                  char *msg)
{
    ...
    sol->stat=SOLQ_NONE;            // spp 输出设置为没有输出 ？？？
    
    if (n<=0) {
        strcpy(msg,"no observation data");
        return 0;
    }
    sol->time=obs[0].time;          // 输出设置为接收机时间
    msg[0]='\0';                    // '\0' 字符串的结束标志
    ...
    if (opt_.mode!=PMODE_SINGLE) { /* for precise positioning ppp会先调用spp mode->6 */
        // opt_.sateph = EPHOPT_BRDC; // 北斗ppp的spp必须使用广播星历，否则通不过卡方检验  (自己添加的)
        opt_.ionoopt=IONOOPT_BRDC;    // 电离层改正采用广播星历Klobchar 模型     
        opt_.tropopt=TROPOPT_SAAS;    // 对流层改正采用Saastamoinen模型
    }
    /* satellite positions, velocities and clocks */
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);
    
    /* estimate receiver position and time with pseudorange */
    stat=estpos(obs,n,rs,dts,var,svh,nav,&opt_,ssat,sol,azel_,vsat,resp,msg); // spp 计算测站位置
    
    /* RAIM FDE 接收机自主正直性检测 只对参与定位的卫星进行检测 */
    if (!stat&&n>=6&&opt->posopt[4]) { // estpos()定位不良 + >= 6颗可见卫星+？
    ...
    if (ssat) { // 输出卫星状态
        ...
        for (i=0;i<n;i++) {
            ...
            ssat[obs[i].sat-1].resp[0]=resp[i]; //输出残差序列
        }
    }
