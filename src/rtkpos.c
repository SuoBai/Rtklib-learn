...
* precise positioning
* args   : rtk_t *rtk       IO  RTK control/result struct  中间信息
*            rtk->sol       IO  solution
...
{   // 开始算法
    ...
    if (opt->refpos<=POSOPT_RINEX&&opt->mode!=PMODE_SINGLE&&
        ...
        for (i=0;i<6;i++) rtk->rb[i]=i<3?opt->rb[i]:0.0;    // 起初全置0
    }
    /* count rover/base station observations */
    for (nu=0;nu   <n&&obs[nu   ].rcv==1;nu++) ;            // 确定流动站的观测卫星数
    for (nr=0;nu+nr<n&&obs[nu+nr].rcv==2;nr++) ;            // 确定基准值的观测卫星数
    ...
    if (time.time!=0) rtk->tt=timediff(rtk->sol.time,time);     // 采样间隔
