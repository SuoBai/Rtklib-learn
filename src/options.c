...
EXPORT opt_t sysopts[]={
    {"pos1-posmode",    3,  (void *)&prcopt_.mode,       MODOPT }, // 将配置存放在全局变量 prcopt_ 结构体中
...
extern opt_t *searchopt(const char *name, const opt_t *opts)
{
    ...
    for (i=0;*opts[i].name;i++) {
        if (strstr(opts[i].name,name)) return (opt_t *)(opts+i); // 返回 ？？？
    }
    return NULL;
}
...
extern int loadopts(const char *file, opt_t *opts)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        *p++ = '\0'; // 让'='为'\0'
        chop(buff);  // 再次去除空格
        if (!(opt=searchopt(buff,opts))) continue; // 将buffer=pos1-posmode传入
