                 ...
                 int m, double *X)
{
 ...
 /* 返回：C=alpha*A*B+beta+C   alpha & beta 是数 A、B是相乘的两个矩阵 tr是转置的标志
 * n,k,m表示两个矩阵的大小（两个矩阵相乘，前矩阵的列 = 后矩阵的行，故三个参数足矣）
 * n:第一个矩阵的行；m第一个矩阵的列/第二个矩阵的行；k第二个矩阵的列
 * 作者之所以把n,k放在m前面，其实是矩阵相乘一共要循环n*k次，从编程的顺序来排的
 */
/* multiply matrix -----------------------------------------------------------*/
extern void matmul(const char *tr, int n, int k, int m,
                   const double *A, const double *B, double *C)
    ...
    switch (f) {
        case 0: /* NN 不转置 */
          for (int j=0;j<k;j++) {
              for (int i=0;i<n;i++) {
                  double d=0.0;
                  for (int x=0;x<m;x++) d+=A[i+x*n]*B[x+j*m]; // A*B 注意矩阵的脚标是竖着排的，这里是左行*右列，左行的逐个*右列并
                  C[i+j*n]=d;
              }
          }
          break;
               ...
               double *Q)
{
    ...
    if (m<n) return -1;
    Ay=mat(n,1);
    matmul("NN",n,1,m,A,y,Ay); /* Ay=A*y BPL */
    matmul("NT",n,n,m,A,A,Q);  /* Q=A*A'  B'PB*/
    if (!(info=matinv(Q,n))) matmul("NN",n,1,n,Q,Ay,x); /* x=Q^-1*Ay */ // 求逆
    free(Ay);
    return info;
}
/* kalman filter ---------------------------------------------------------------
