                    ...
                    rtk_t *rtk, int mode)
{
    ...
    // rtk_t rtk; // 存储中间变量
    obsd_t *obs_ptr = (obsd_t *)malloc(sizeof(obsd_t)*MAXOBS*2); /* for rover and base = 192 */
    double rb[3]={0};
    ...
    while ((nobs=inputobs(obs_ptr,rtk->sol.stat,popt))>=0) { // 读o文件一个历元下所以的卫星数据，将全局变量obss读取到局部变量obs ,在这里就可以控制仿实时的流程

        /* exclude satellites */
        for (i=n=0;i<nobs;i++) {
            if ((satsys(obs_ptr[i].sat,NULL)&popt->navsys)&&
                popt->exsats[obs_ptr[i].sat-1]!=1)  // 根据设置的卫星系统和废弃卫星编号删除无用obs
                obs_ptr[n++]= obs_ptr[i];           // 比如有47颗卫星，但只设置了10颗北斗，则此步会把sbs[0]至sbs[9]全部用来存这10颗卫星
                                                    // 相当于将北斗卫星数据前置，后续只用这10个元素，第11到...也有数据但无用，第48-192全为空
        }
        if (n<=0) continue;                         // 本历元观测到了几颗目标系统的卫星

        /* carrier-phase bias correction 使用FCB文件进行载波相位改正 */
        if (!strstr(popt->pppopt,"-ENA_FCB")) {
            corr_phase_bias_ssr(obs_ptr,n,&navs);   // corrent phase bias
        }
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    /* read erp data */ 
    // 地球自转参数，Earth Rotation Parameters，ERP
    if (*fopt->eop) {
    ...
    /* read dcb parameters from DCB, BIA, BSX(混合系统) files */
    dcb_ok = 0;
                     ...
                     const char *rov)
{
    ...
    if (i<n) { // 多个流动站 先设置一个 /* include rover keywords */
                     ...
                     const char *rov, const char *base)
{
    ...
    // 读取sp3、clk、FCB、PPP改正信息文件、sbas，并初始化rtcm
