...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    for (i=0;i<2;i++) { // 经2轮卫星钟差修正ts，求取最佳的时间间隔并存于t（迭代）， 丁的程序无这步
    ...
    return eph->f0+eph->f1*t+eph->f2*t*t; // 用求得的最佳间隔t求卫星钟差
}
                    ...
                    double *var)
{
    ...
    switch ((sys=satsys(eph->sat,&prn))) {
        ...
        case SYS_CMP: mu=MU_CMP; omge=OMGE_CMP; break; // 地球引力常数和地球自传角速度
        default:      mu=MU_GPS; omge=OMGE;     break;
    }
    M=eph->M0+(sqrt(mu/(eph->A*eph->A*eph->A))+eph->deln)*tk; // 平近点角

    for (n=0,E=M,Ek=0.0;fabs(E-Ek)  >RTOL_KEPLER&&n<MAX_ITER_KEPLER;n++) { // 迭代结算近点角
    ...
    u+=eph->cus*sin2u+eph->cuc*cos2u; //改正后的维度参数
    r+=eph->crs*sin2u+eph->crc*cos2u; // 改正后的维度向径
    i+=eph->cis*sin2u+eph->cic*cos2u; // 改正后的倾角
    x=r*cos(u); y=r*sin(u); cosi=cos(i);

    /* beidou geo satellite */ // bds和geo与众不同 
    if (sys==SYS_CMP&&(prn<=5||prn>=59)) { /* ref [9] table 4-1 */
        O=eph->OMG0+eph->OMGd*tk-omge*eph->toes; // 改正升交点的经度
        sinO=sin(O); cosO=cos(O);
        xg=x*cosO-y*cosi*sinO; // 卫星在地固系钟的坐标
        yg=x*sinO+y*cosi*cosO;
        zg=y*sin(i);
        sino=sin(omge*tk); coso=cos(omge*tk);
        rs[0]= xg*coso+yg*sino*COS_5+zg*sino*SIN_5; // 北斗IGSO\MEO与GPS坐标计算相同
        rs[1]=-xg*sino+yg*coso*COS_5+zg*coso*SIN_5; // 在ecef框架中 geo卫星的轨道倾角为零
        rs[2]=-yg*SIN_5+zg*COS_5; // 因此升交点的赤经不能与近地点的自变量分开，因此在计算钟引入了5度的旋转
    }
    else {
    ...
    tk=timediff(time,eph->toc); //此处times是经过之前多次钟差改正后求得的信号发射时的卫星钟面时刻最优解
    *dts=eph->f0+eph->f1*tk+eph->f2*tk*tk;  // 求最优解时刻下的卫星钟差

    /* relativity correction */
    *dts-=2.0*sqrt(mu*eph->A)*eph->e*sinE/SQR(CLIGHT);  // 相对论效应改正

    /* position and clock error variance */
    *var=var_uraeph(sys,eph->sva); //定位与钟方差
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    sys=satsys(sat,NULL);
    switch (sys) { // 根据系统设置 o n 文件允许的时间差阈值tmax 
        case SYS_GPS: tmax=MAXDTOE+1.0    ; sel=eph_sel[0]; break; // 都预置0
    ...
    for (i=0;i<nav->n;i++) {    // 遍历所有星历， 北斗和GPS对iode的定义不同？
        if (nav->eph[i].sat!=sat) continue; // 筛选星历-相同的prn
        if (iode>=0&&nav->eph[i].iode!=iode) continue;  // 筛选星历-相同的iode；ephclk()调用本函数时传入的iode为-1，则无需检查iode
        if (sys==SYS_GAL) {
        ...
        if ((t=fabs(timediff(nav->eph[i].toe,time)))>tmax) continue;    // 筛选星历-n o文件时间间隔所设阈值
        if (iode>=0) return nav->eph+i; // 星历中的iode正常都>0,eph指向数组元素的首地址，+i则指向当前循环到的元素
        // iode<0时，除需满足低于阈值，还需要找出间隔最小的星历，用j记录位置
        if (t<=tmin) {j=i; tmin=t;} /* toe closest to time */   // 更新离o文件时刻最近的toe记录
    }
    if (iode>=0||j<0) { // iode都>0但无任何的toe与o文件时间间隔<阈值，星历匹配失败
    ...
    // 至此传入的iode必<0，返回的星历不仅满足间隔小于阈值，还是最靠近o文件时间的
    return nav->eph+j;

    /*
        该函数的效果： 若传入的iode>0(相当于要根据iode匹配星历)，则在导航数据中找同期（同iode）的数据
                     若传入的iode<0*(相当于不根据iode匹配星历)，则不经要找间隔低于阈值的时刻，还要找间隔最小即靠近o文件时刻的星历数据
     */
}
                  ...
                  double *dts)
{
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        if (!(eph=seleph(teph,sat,-1,nav))) return 0; // 匹配星历，传参iode=-1<0,会跳过iode检查，返回靠近o文件时间的星历未匹配到时间间隔阈值内的星历数据跳至此，星历匹配成功
        *dts=eph2clk(time,eph); // 计算卫星钟差dts  北斗的钟差可否用此函数计算？
    }
    else if (sys==SYS_GLO) {
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        ...
        time=timeadd(time,tt); // 人为添加时间增量，得卫星的新位置和新钟差（通过扰动法计算卫星的位置、速度和钟漂，注意不是接收机的钟差）
        eph2pos(time,eph,rst,dtst,var); // 计算卫星新位置和新钟差存入rst和dst
        *svh=eph->svh; 
    }
    else if (sys==SYS_GLO) {
    ...
    for (i=0;i<3;i++) rs[i+3]=(rst[i]-rs[i])/tt;  // 卫星速度
                  ...
                  int *svh)
{
    ...
    *svh=0;
    ...
    switch (ephopt) {
        ...
        case EPHOPT_PREC  : // conf文件选择精密星历时
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        for (j=0;j<6;j++) rs [j+i*6]=0.0;       // 第i行全部置为0
        for (j=0;j<2;j++) dts[j+i*2]=0.0;
        var[i]=0.0; svh[i]=0;

        /* search any pseudorange */
        for (j=0,pr=0.0;j<NFREQ;j++) if ((pr=obs[i].P[j])!=0.0) break;  // 遍历各频点，寻找非0可用的伪距
        ...
        time[i]=timeadd(obs[i].time,-pr/CLIGHT);    // 接收机t-信号传播用时约（0.075秒）≈ 卫星信号的发射时刻（gpst）
        // 0.075的量级>>卫星和接收机钟差，并且接收机t还未改正dtr，因此time[i]是一个粗略的信号发射的卫星钟面时刻

        /* satellite clock bias by broadcast ephemeris */
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) { // 由广播星历计算卫星发射信号的钟差
            trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
             // printf("no broadcast clock %s sat=%2d\n", time_str(time[i],3),obs[i].sat);
            continue;
        }
        time[i]=timeadd(time[i],-dt);  // 卫星钟差改正（钟面t-钟差 = 信号发布时卫星钟真值）

        /* satellite position and clock at transmission time */
        // rs 每一列有6个元素 x y z vx vy vz; dts 每一列有两个元素钟差和钟速
